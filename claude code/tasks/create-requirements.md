[Input]
  1. "sunnycore/templates/requirement-tmpl.yaml" --Requirement template
  2. User-provided initial ideas and requirement descriptions

[Output]
  1. "docs/requirements/*.md" --Complete requirement specifications (including functional requirements, non-functional requirements, and acceptance criteria)
...
[Steps]
  ...
  5. Finalization Phase
    ...
    - Populate requirement template and prepare draft content for "docs/requirements.md"
    ...
    - if user approves draft then proceed to 5.1, else proceed to 5.2
      ...
      5.1. Write Final Documents
        - Write approved content to "docs/requirements.md"
        - Execute uv run "sunnycore/scripts/shard-requirements.py"
        - if execution succeeds then proceed to verification, else log error to "logs/errors.log" (including timestamp, error type, failure reason, impact scope) and notify the user
...
[DoD]
  ...
  - [ ] "shard-requirements.py" has been executed or errors have been logged to "logs/errors.log" and user has been notified
