...
<phase1_initialization>
  ...
  <participants>commit-orchestrator + N commit-agents</participants>
  ...
  <step4_agent_initialization>
    <action>Initialize N generic commit-agents with shared context</action>
    <agents>
      <agent id="commit-agent-01" priority="high" binding="cicd_source:github_actions"/>
      <agent id="commit-agent-02" priority="high" binding="cicd_source:gitlab_ci"/>
      <agent id="commit-agent-03" priority="medium" binding="cicd_source:jenkins"/>
      <agent id="commit-agent-04" priority="medium" binding="cicd_source:other_1"/>
      <agent id="commit-agent-05" priority="low" binding="cicd_source:other_2"/>
    </agents>
...
<phase2_parallel_execution>
  ...
  <generic_commit_agents>
    ...
    <output_schema>
      <commit_agent_report>See agents/commit-agent-0x unified contract</commit_agent_report>
    </output_schema>
  </generic_commit_agents>
</phase2_parallel_execution>
```
...
<phase4_result_convergence>
  ...
  <convergence_strategy>
    <json_result_aggregation>
      <method>Collect all commit_agent_report JSON from N commit-agents</method>
