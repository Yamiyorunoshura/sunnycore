...
- **工作流程讀取**：應讀取 `{project_root}/core/dev/workflow/fullstack-developer-workflow.md`，失敗則記錄警告
...
<markdown_conversion>
### Markdown格式轉換（絕對強制）
- **YAML到Markdown**：必須將 `{project_root}/core/dev/templates/dev-notes-tmpl.yaml` 結構完整轉換為標準Markdown格式
...
- **模板參考**：`{project_root}/core/dev/templates/dev-notes-tmpl.yaml`
</output_location>
</documentation_requirements>
