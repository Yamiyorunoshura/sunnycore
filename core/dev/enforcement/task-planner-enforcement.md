...
- **工作流程讀取**：應讀取 `{project_root}/core/dev/workflow/unified-task-planning-workflow.md`，失敗則記錄警告
- **範本讀取**：應讀取 `{project_root}/core/dev/templates/implementation-plan-tmpl.yaml`，失敗則記錄警告
...
- **結構一致性**：應符合 `{project_root}/core/dev/templates/implementation-plan-tmpl.yaml` 結構；不一致時記錄差異
