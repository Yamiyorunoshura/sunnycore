...
- **工作流程讀取**：應讀取 `{project_root}/core/po/workflow/unified-architecture-documentation-workflow.yaml`，失敗則記錄警告
- **模板讀取**：應讀取 `{project_root}/core/po/templates/architecture-doc-tmpl.yaml`，失敗則記錄警告
...
### 文檔完整性（強制執行）
- **結構合規**：必須遵循 `{project_root}/core/po/templates/architecture-doc-tmpl.yaml` 的結構
