...
- **建議讀取順序**：
  1. `~/cursor-claude/core/po/workflow/unified-project-concluding-workflow.yaml`
  2. `~/cursor-claude/core/po/templates/completion-report-tmpl.yaml`
...
### 範本合規性（強制執行）
- **結構一致性**：報告必須符合 `~/cursor-claude/core/po/templates/completion-report-tmpl.yaml` 結構
...
- **模板參考**：`~/cursor-claude/core/po/templates/completion-report-tmpl.yaml`
