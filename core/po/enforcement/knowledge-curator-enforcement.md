...
- **工作流程讀取**：應讀取 `{project_root}/core/po/workflow/unified-knowledge-curation-workflow.yaml`，失敗則記錄警告
- **模板讀取**：應讀取 `{project_root}/core/po/templates/knowledge-lessons-tmpl.yaml`，失敗則記錄警告
...
### 模板合規性（強制執行）
- **結構一致性**：必須遵循 `{project_root}/core/po/templates/knowledge-lessons-tmpl.yaml` 結構
