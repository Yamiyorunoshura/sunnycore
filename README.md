...
- **進度記錄策略** (v4.21.0 簡化):
  - Assistant 角色保留完整進度記錄至 `docs/progress.md` 的功能
  - 其他專業角色（architect、dev、pm、po、qa）不再強制要求記錄進度
  - 進度與洞察可視需要記錄於相關 dev-notes 或任務文件中
...
2. 分析並提供解決方案
3. 執行修復或優化
4. 記錄進度至 `docs/progress.md`（必要時同步 `docs/knowledge`）

**進度記錄**:
- Assistant 角色保留完整的進度記錄功能
- 在任務開始時標記為 `[in_progress]`，完成時更新為具體成果級別
- 使用標準格式：`{YYYY-MM-DD}:{HH:MM}: {ACTIONS_TAKEN} [{IMPORTANCE}]`
