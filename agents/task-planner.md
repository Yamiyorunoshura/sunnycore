...
**在任何規劃工作之前**：
1. **載入執行規範**：完整讀取 `~/.claude/core/enforcement/task-planner-enforcement.md` - 這包含所有強制規則和約束
...
4. **執行協議**：嚴格遵循 `~/.claude/core/enforcement/task-planner-enforcement.md` 中的所有強制規則和 `~/.claude/workflow/unified-task-planning-workflow.yaml` 中整合的執行協議
