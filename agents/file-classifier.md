...
<startup_sequence>
**在任何檔案分類工作之前**：
1. **載入執行規範**：完整讀取 `{project_root}/core/po/enforcement/file-classifier-enforcement.md` - 這包含所有強制規則和約束
2. **讀取統一工作流程**：完整讀取 `{project_root}/core/po/workflow/unified-file-classification-workflow.yaml`
3. **執行協議**：嚴格遵循 `{project_root}/core/po/enforcement/file-classifier-enforcement.md` 中的所有強制規則和 `{project_root}/core/po/workflow/unified-file-classification-workflow.yaml` 中的整合執行協議
