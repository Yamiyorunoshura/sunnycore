...
- 修復了config提示詞中example過於詳細影響LLM判斷的問題
...
### Added
- QA 角色質量指標體系：新增 `[QA-Key-Metrics]` 章節至所有平台的 QA 命令文件
  - 測試覆蓋率指標：包含行覆蓋率（80%+）、分支覆蓋率（75%+）、函數覆蓋率（85%+）和整合覆蓋率標準
  - 測試通過率標準：單元測試（100%）、整合測試（98%+）、E2E 測試（95%+）、回歸測試（100%）
  - 代碼質量指標：環狀複雜度（< 10）、重複率（< 3%）、代碼異味（零關鍵）、技術債比率（< 5%）
  - 缺陷指標：缺陷密度（< 1/1000 行）、關鍵 bug 數量（零）、bug 修復率（95%+）、逃逸缺陷（< 2%）
  - 架構合規性：設計模式遵循（100%）、API 契約合規（100%）、安全標準（零關鍵漏洞）、性能標準（符合 SLA）
  - 性能指標：響應時間（95th < SLA）、吞吐量、資源利用率（CPU < 70%、記憶體 < 80%）、錯誤率（< 0.1%）
  - 審查質量指標：審查覆蓋率（100%）、審查周轉時間（24-48 小時）、可執行發現（> 80%）、誤報率（< 10%）
...
### Added
- Assistant 角色計劃工作流程與知識管理功能：為 `commands/sunnycore_assistant.md` 新增強制性計劃流程和知識管理機制
  - 新增 `[Planning-Workflow]` 章節：定義計劃觸發、內容要求、維護和完成檢查流程
    * 要求在執行任何任務前必須先生成書面計劃（至少三步）
    * 計劃必須包含預期輸出、負責人、所需產物和成功標準
    * 計劃變更時必須先更新計劃並記錄理由
    * 提供標準化的計劃提示格式模板
  - 新增 `[Knowledge-Management]` 章節：定義如何使用和維護 curated knowledge
    * 要求將 `docs/knowledge` 視為權威知識庫，方案提出前先掃描相關條目
    * 要求在計劃和交付物中引用知識來源（格式：`docs/knowledge/{file}.md [Section]`）
    * 定義新知識捕獲的語義結構（best-practices、errors、problem-solving）
    * 要求保留衝突實踐並記錄適用情境，而非丟棄替代方案
  - 更新 `[Constraints]` 章節：從 7 條擴充到 9 條，所有約束重新改寫以提升明確性
    * 第 1 條：強化計劃格式要求，必須使用 [Planning-Workflow] 定義的格式
    * 第 2 條：強化計劃遵循要求，變更時必須先更新計劃並記錄理由
    * 第 3 條（新增）：要求遵循 `docs/architecture/` 中的系統架構，未經架構師批准不得引入衝突模式
    * 第 4 條（新增）：要求諮詢並引用 curated knowledge，禁止捏造或忽略既有證據
    * 第 5-9 條：重新表述原有約束，提升可操作性和明確性
  - 更新 `[Examples]` 為正反對比格式：從 3 個簡單範例改為 2 Good + 2 Bad 範例系統
    * Good Example 1（認證回歸修復）：展示正確的計劃驅動、知識引用、架構遵循流程
    * Good Example 2（分析效能劣化）：展示正確的效能優化流程與知識庫整合
    * Bad Example 1（無計劃無知識的修補）：展示違反約束 1、2、4 的錯誤做法及正確方法
    * Bad Example 2（修復時的架構偏離）：展示違反約束 3、7 的錯誤做法及正確方法
  - 更新 `[Input]` 格式：明確指定檔案格式 `docs/knowledge/*.md` 和新增 `docs/architecture/*.md`
  - 與 v4.5.0（配置文件範例增強）和 v4.6.0（任務文件範例增強）保持一致，形成完整的正反對比教學體系
  - 提升 Assistant 角色的執行規範性、架構完整性和知識復用能力
...
### Added
- Dev 角色架構遵循約束：為 `commands/sunnycore_dev.md` 新增第 4 條約束規則
  - 要求 Dev 角色必須遵循當前架構設計進行任何實作
  - 禁止創建偏離當前架構的實作計劃或代碼
...
### Changed
- 優化安裝腳本性能：改進 `scripts/install.py` 的目錄掃描效率與錯誤處理
  - 新增 `collect_directory_files_parallel` 方法：支援並行收集多個目錄中的所有文件
    * 動態調整並行數量：根據目錄數量自動調整（預設最多 20 個並行任務）
    * 線程安全設計：使用 Lock 機制確保並行收集的數據一致性
  - 優化空目錄處理：從返回 False 改為顯示警告並返回 True，避免空目錄導致安裝失敗
  - 重構 `install_cursor` 和 `install_claude` 方法：將順序掃描改為並行掃描
    * 新增掃描進度提示：顯示使用的並行任務數量與掃描結果統計
    * 提升安裝速度：多目錄同時掃描，減少總體等待時間
  - 提升安裝腳本的執行效率與用戶體驗
